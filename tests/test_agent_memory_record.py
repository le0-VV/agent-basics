...
""",
            ...
            self.assertRegex(decision_files[0].name, r"^\d+-use-rust-binary\.md$")
            self.assertRegex(decision_text, r"(?m)^id: decision-\d+-use-rust-binary$")
            self.assertRegex(decision_text, r"(?m)^created: \d+$")
            self.assertRegex(decision_text, r"(?m)^updated: \d+$")
