...
`agent-basics` does not vendor OpenViking, model weights, or Python package dependencies into this repository or into the Rust wrapper binary. The installer and helper commands can install or call those tools on the user's machine, so their separate terms still apply.
...
| --- | --- | --- |
| OpenViking | Installed into a user-level virtualenv, normally `~/.openviking`, and called as an external CLI/server. agent-basics may package the user-level server entrypoint into `~/.openviking/openviking` on the user's machine so the service has a clear process name. It is not vendored into this repo. | OpenViking's main project is AGPL-3.0. The upstream repo also documents `ov_cli` under Apache-2.0. If you modify, redistribute, package, or network-host OpenViking, comply with OpenViking's upstream license terms. Source: <https://github.com/volcengine/OpenViking> |
...
| PyInstaller | Used on the user's machine to package the local MLX Python server into a standalone `agent-basics-mlx` executable and the user-level OpenViking server into `~/.openviking/openviking`. The generated executables may include resolved Python packages from their user-level virtualenvs. | PyInstaller is GPL-2.0-or-later with a bootloader exception. Before redistributing a generated executable, audit the bundled dependency set and include required notices. Source: <https://pyinstaller.org/> |
...
- Do not vendor OpenViking source, MLX runtime packages, model weights, generated virtualenvs, or generated PyInstaller bundles into this repository.
- Do not describe OpenViking, MLX packages, Gemma, EmbeddingGemma, or optional Python packages as covered by the agent-basics MIT license.
